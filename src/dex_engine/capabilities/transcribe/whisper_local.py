"""whisper-local: faster-whisper on CPU int8 — the free transcription floor.
...
faster-whisper is lazy-imported inside methods: tests inject a fake
...
def load_whisper_model(model: str) -> LoadedModel:
    """Load (downloading on first use) a faster-whisper model, CPU int8.
    ...
    Returns:
        The loaded model.
    """
    from faster_whisper import WhisperModel  # noqa: PLC0415 — lazy: heavy dep
...
def _load_failure_types() -> tuple[type[Exception], ...]:
    """The load-time failure classes that mean "not available right now".
    ...
    they are named explicitly; the import is lazy like every heavy dep.
    """
    base: tuple[type[Exception], ...] = (OSError, ValueError, RuntimeError)
    try:
        from huggingface_hub.errors import HfHubHTTPError  # noqa: PLC0415 — lazy
...
class WhisperLocal:
        ...
        *,
        ...
        Args:
            ...
            load: model name -> loaded model; the real one builds a
                ``WhisperModel`` (tests inject a fake).
    ...
    def available(self) -> Availability:
        ...
        try:
            ...
            # A broken native install parks jobs with the reason; the
    ...
    def transcribe(self, audio: Path, initial_prompt: str) -> str:
        """Transcribe one audio file, primed with the item's vocabulary.
