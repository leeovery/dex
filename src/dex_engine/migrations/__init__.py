"""Migrations: mechanical, near-instant state rewrites, run by sync first (§12).
...
engine version are injected (§14), never read ambiently.
...
state JSONL (§4). The log is the fast path; **idempotency is the backstop**:
...
Authoring rules (§12): mechanical and near-instant only. Exactly two
...
class Migration(Protocol):
    """One migration (§12): a number, a stated intent, and a mechanical apply."""
...
def log_path(root: Path) -> Path:
    """``state/migrations.jsonl`` under the instance at ``root`` (§4)."""
...
def discover(
    *,
    ...
    the runner sorts numerically (§12).
    ...
    for module_info in pkgutil.iter_modules(__path__):
        ...
        if migration.number != number:
            raise MigrationError(
                ...
                f"but its filename says {number} — the filename is the registry (§12)"
            )
...
def read_applied(path: Path) -> set[int]:
    ...
    The file merges as a union between machines (§4), so duplicate records
    ...
    for lineno, line in enumerate(path.read_text(encoding="utf-8").split("\n"), start=1):
        ...
        if not isinstance(number, int) or isinstance(number, bool):
            raise MigrationError(
                ...
                f"({{number, engine, date}}, §12): {line!r}"
            )
        applied.add(number)
    return applied
...
def run_pending(
    ...
    *,
    ...
    Called by sync before anything else touches state (§12). The log record
