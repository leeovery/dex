...
Membership is every pre-rewrite ``done`` web/x **page** entry — never a
``job`` unit: a media download or asset write is a byproduct of its
parent's fetch, has no links to keep and no thread to walk, and is
regenerated by the parent's own rerun — **whose work a live corpus item
still claims**: this migration runs directly after migration 1,
...
def _in_cohort(entry: LedgerEntry, *, skipped: list[Skipped]) -> bool:
    ...
    Media downloads and asset writes are never members, whatever their
    vintage: both fixes are about page CONTENT (links kept, threads
    walked), and a byte-write has neither. They carry the parent page's
    kind, so without this the sweep requeued them as plain page reruns —
    job and lineage stripped — sending a signed media URL and an asset
    repo path to the web driver on the next run. Re-fetching the parent
    page regenerates its media children anyway.
    """
