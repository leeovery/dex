...
class FileDriver:
    ...
    def fetch(self, unit: WorkUnit) -> Outcome:
        ...
        if data.startswith(_LFS_POINTER_PREFIX):
            return Unusable(
                evidence=f"{name or 'file'} is an unsmudged LFS pointer — run `git lfs pull`"
            )
        ...
        if _rerouteable(unit.url) and sniff_format(data) is None and looks_like_html(data):
            return Redetected(kind=Kind.WEB)
    ...
    def _read_local(self, repo_path: str) -> tuple[bytes, str | None] | Outcome:
        ...
        if path is None:
            return Unusable(
                evidence=f"{repo_path} points outside the instance root — heal the capture"
            )
        if not path.is_file():
            return Unusable(
                evidence=(
    ...
    def _download(self, url: str) -> tuple[bytes, str | None] | Outcome:
    ...
    def _extract(self, data: bytes, fmt: Format, name: str | None) -> Outcome:
        extractor = self._capabilities.extractor(fmt)
        if extractor is None:
            # No mechanical provider for THIS format: a capability park,
            # with the registry's stated reason — the cognitive floor
            # surfaces it on the report for the session.
            availability = self._capabilities.available(Need.EXTRACT, fmt)
            return NeedsCapability(need=Need.EXTRACT, reason=availability.reason)
        try:
            ...
            return NeedsCapability(need=Need.OCR, reason=str(e))
            ...
            # world's weather.
            return NeedsCapability(need=Need.EXTRACT, reason=str(e))
        meta: dict[str, str | int | None] = {
            "title": name,
            "format": fmt.value,
            "via": extractor.name,
        }
        return Content(meta=meta, body=extraction.markdown, assets=extraction.assets)
